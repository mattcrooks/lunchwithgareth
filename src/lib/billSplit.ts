...
export class BillSplitService {
  ...
  async createBillSplitRequest(
    ...
    try {
      ...
      const privateKeyHex = await CryptoManager.decryptData(
        encryptedPrivateKey,
        devicePassword
      );
      ...
      const unsignedEvent = this.eventManager.createPaymentRequestEvent(
        eventData,
        storedKey.pubkey
      );
      ...
      this.eventManager.setRelays(
        relayManager.getWriteRelays().map((r) => r.url)
      );
  ...
  async markParticipantPaid(
    ...
    try {
      ...
      await storage.updateParticipantPayment(
        receiptId,
        pubkey,
        paidSats,
        method
